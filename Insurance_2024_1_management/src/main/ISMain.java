...
//			} 
	...
	private void showLoanApplicationList() {
		int index = 1;
		System.out.println();
		System.out.println("-- Employee List --");
		for(LoanApplication loanApplication : loanApplicationListImpl.retrieveAll()) {
			System.out.println(index + ". ID: " + loanApplication.getLoanApplicationID() + " customer ID: " + loanApplication.getCustomerID());
			index++;
		}
	}
	// 대출을 신청한다.
	...
	private static void printEmployeeMainMenu() {
		...
		System.out.println("9. 경영전략 수립");
		System.out.println("10. 사규 생성");
	...
	private void startEmployeeService() throws IOException, ParseException, AuthenticationException, AuthorizationException{
		while(true) {
			...
			try {
				...
				else if (clientChoice.equals("9")) createBusinessStraegy();
				else if (clientChoice.equals("10")) createRegulation();
	...
	// 대출신청을 접수한다.
	private void loanTakeOver() throws IOException {
		int index = 1;
		...
		System.out.println("-- 신청된 대출 리스트. --");
		for(LoanApplication loanApplication : permitLoanApplicationList) {
			System.out.println(index + ". " + "계약 id: "+loanApplication.getLoanApplicationID());
			index++;
		}
		System.out.println("-----------------------------");
		System.out.print("접수할 대출 id: "); String loanApplicationID = dataValidation(clientInputReader.readLine().trim(), "String");
		System.out.println(employee.loanTakeOver(loanApplicationID));
		loanApplicationReview(loanApplicationID);
	}
	// 대출 신청 심사를 요청한다.
	private void loanApplicationReview(String loanApplicationID) throws IOException {
	...
	private void loanJudge() throws IOException {
		...
		if(requestedApplicationList.size() == 0) {
			System.out.println("심사요청된 대출 내역이 존재하지 않습니다.");
			return;
		}
		System.out.println("-- 심사요청된 대출 리스트. --");
		for(LoanApplication loanApplication : requestedApplicationList) {
			System.out.println(index + ". " + "계약 id: "+loanApplication.getLoanApplicationID());
			index++;
		}
		System.out.println("-----------------------------");
		System.out.print("심사할 대출 id: "); String loanApplicationID = dataValidation(clientInputReader.readLine().trim(), "String");
		LoanApplication loanApplication = loanApplicationListImpl.retrieveById(Integer.parseInt(loanApplicationID));
	...
	private void reviewProcessForAssetManagement() throws IOException {
		...
		for(AssetManagement assetManagement : requestedAssetManagementStatus) {
			System.out.println(index + ". " + "계약 id: "+assetManagement.getAssetManagementID());
			index++;
		}
		System.out.println("-----------------------------");
	...
	private void createBusinessStraegy() throws AuthenticationException, IOException, DuplicateIDException {
	    ...
	    BusinessStraegy businessStraegy = new BusinessStraegy();
	...
	private void assignmentManagement() throws IOException {
        ...
        System.out.print("직책: "); String newPosition = dataValidation(clientInputReader.readLine().trim(), "String");
	...
	// 지급 요청에 따라 지급한다.
	private void disburseLoanAmount() throws IOException {
		int index = 1;
