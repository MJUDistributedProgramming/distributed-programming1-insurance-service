...
public class LoanListImpl implements LoanList {
    ...
    public Loan m_Loan;
    public LoanListImpl(){loanList = new ArrayList<>();}
    public void finalize() throws Throwable {}
    public String add(Loan Loan) throws DuplicateIDException {
        for (Loan loan : loanList) {
            if (loan.getLoanID() == Loan.getLoanID()) throw new DuplicateIDException();
        }
    ...
    public String deleteById(int id){
        ...
        for (Loan loan : loanList) {
            if (loan.getLoanID() == id) deleteLoan = loan;
        }
        if (deleteLoan == null) return "[error] 해당 id의 대출정보가 존재하지 않습니다.";
        loanList.remove(deleteLoan);
        return "[success] 해당 대출정보가 삭제되었습니다.";
    }
    public ArrayList<Loan> retrieveAll(){return loanList;}
